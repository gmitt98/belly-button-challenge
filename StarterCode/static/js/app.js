...
// 2. Put the otu_id and their corresponding sample_values into bar chart for each SUbject ID
  d3.json(url).then(function(data) {
    ...
    // Create a chart that we will update with actual data, we start it with the first Subject ID that we have (940 in this case)
...
function updateCharts(id, data) {
    ...
    let otu_ids = sample.otu_ids.slice(0, 10).map(otu_id => `OTU ${otu_id}`) //.reverse(); // Gets the top 10 OTUs
...
d3.selectAll("#selDataset").on("change", function() { // When the dropdown list is changed, i.e. new option selected...
    ...
    d3.json(url).then(function(data) {
        updateCharts(newId, data); // Run the update function we wrote
    });
});
